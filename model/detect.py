...
from regression.regression import Processor
...
def toyolo_format(original_coor):
    ...
    YMax= original_coor[3]
    yolo_format = [(XMin+XMax)/(2*W), (YMin+YMax)/(2*H), (XMax-XMin)/(W), (YMax-YMin)/(H)]
    # [260.50000, 104.43750, 530.50000, 355.75000,   0.92676,   2.00000]
    # 左上mao點寬座標,左上mao點高座標,右下mao點寬座標,右下mao點高座標
    return [float(i) for i in yolo_format]
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for i,item in enumerate(det):
                    ...
                    #    TO_regresssion[class_name] =  { 'number':1, 'coordinate':[coor]}
                    #else:
                    #    TO_regresssion[class_name]['number'] += 1
                ...
                #evaluate price
                #output price = evaluate_price(TO_regresssion)
